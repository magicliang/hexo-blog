...
避免这种幻读的情况是在事务开始以前提前求 S 或者 X 锁，例子见[《對於 MySQL Repeatable Read Isolation 常見的三個誤解》][4]。MVCC 能够在对单行锁定的时候实现防止插入，就是部分解决幻读的效果，但如果单纯地重复让两个事务并发执行，事务 1 读一次，事务 2 插入一次，事务 2 再读一次，临键锁并不会被触发，事务 2 的插入不会失败。**只有行锁才会触发间隙锁，进而得到邻键锁，进而触发对幻读的保护。**
