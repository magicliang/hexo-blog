...
InnoDB 在查询和更新的时候，使用 Next-Key-Lock 同时锁定 record + 相关的范围，防止相关查询条件涉及范围内不会插入新的记录被插入，不再出现幻读 - 但这种优化并不是真正序列化，只能保证事务里的 select 不会出现不可重复的结果，事务里有 insert 还是会触发冲突。有人因此[认为 InnoDB 不能阻止幻][1]读，还有[另一个例子][2]。对这个问题的完整解释在[这里][3]。
