...
`LocalDate`是`ChronoLocalDate`的子类（顺序就是这样，没有反过来），但 ChronoLocalDate 已然实现了 `comparable<ChronoLocalDate>`。这时候 LocalDate 的比较方法就应该声明成`<T extends Comparable<? super T>>`- **这是 comparable 的标准泛型方案**。从 A 派生出 B，则 B 的 comparable 方法必须声明为可以支持 super 的类型，这样对 A 的 compare 才能同时兼容 A、B。（泛型集成对比点 2）
...
反射的基础类型是 Type 接口，它有五个子类：
 ...
 - ParameterizedType 接口，描述反省类或接口类型，如 Comparable<? super T> - 奇怪，是 T 而不是 String
...
/**
   ...
   public static void main(String[] args) {
        ...
        final ParameterizedType parameterizedSuperType = (ParameterizedType) genericSuperclass;
        final Type[] actualTypeArguments = parameterizedSuperType.getActualTypeArguments();
        // E
...
[angelikalanger 的 JavaGenericsFAQ][2]。

  [1]: https://github.com/jhalterman/typetools
  [2]: http://www.angelikalanger.com/GenericsFAQ/JavaGenericsFAQ.html#FAQ005
