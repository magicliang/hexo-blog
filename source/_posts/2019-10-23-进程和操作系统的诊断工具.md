...
|符号|含义|
|:--:|:--:|
|18:52:04| 当前时间 up 10days, 3:49 系统运行时间，格式为：天，时:分| |1| user| 当前登录用户数 |
|load average: 0.00, 0.01, 0.05|系统负载，即任务队列的平均长度。三个数值分别为 1分钟、5分钟、15分钟前到现在的平均值|

当CPU完全空闲的时候，平均负荷为0；当CPU工作量饱和的时候，平均负荷为1。

如果CPU每分钟最多处理100个进程，那么系统负荷0.2，意味着CPU在这1分钟里只处理20个进程；系统负荷1.0，意味着CPU在这1分钟里正好处理100个进程；系统负荷1.7，意味着除了CPU正在处理的100个进程以外，还有70个进程正排队等着CPU处理。load 其实是 cpu utilization 的一种具象化表示。load 1.0 大致上等于那段时间里 cpu 的 idle time = 0。注意，**如果有 4 核 cpu，实际上 cpu 的满负荷是 4.0**。
...
[Discover The Possibilities Of The /Proc Directory][4]

[Linux Programmer's Manual PROC(5)][5]
...
**jvm 自己的本地栈大小可以通过指标看出来。**
  ...
  [4]: https://www.linux.com/news/discover-possibilities-proc-directory/
