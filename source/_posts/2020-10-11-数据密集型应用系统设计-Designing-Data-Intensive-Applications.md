...
这个例子可以应用在非常多的 OLAP 场景内，即对于大数据量的数据汇总查询，我们可以优先采取 eager aggregate 或者 broadcast 的方法在写事务的时候插入汇总数据；然后对于 fan-out 特别高的数据，在查询的时候 lazy 查询。选择方案时，权衡考虑的因素主要是写成本比较高，还是读成本比较高。
