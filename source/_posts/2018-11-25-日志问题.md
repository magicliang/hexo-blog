...
Appender：日志输出目的地（output destination），负责日志的输出（输出到什么地方）。有 name，这个 name 可以被引用。是一个很重要的功能接口。
...
AsyncLogger 使用一个 [lock-free data structure][4]（实际上就是 LMAX Disruptor）。
...
1. https://www.jianshu.com/p/9f0c67facbe2
  ...
  [3]: https://logging.apache.org/log4j/2.x/performance.html
